...
后台页面默认字号使用 `13px`，不是 ElementUI 默认的 `14px`。在 `znty-rrs-ui` 中应通过全局样式覆盖表格、表单、按钮、弹窗正文，使密度与切图一致。
...
切图导出物本身不是 ElementUI 源码，但其组件形态与 ElementUI 企业后台高度一致。结合现有 `znty-rrs-ui` 技术栈，组件归类如下：
...
在 `znty-rrs-ui` 中建议建立一个全局样式入口，例如 `css/theme.css` 或现有公共 CSS 文件，集中覆盖 ElementUI：
